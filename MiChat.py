...
from groq import Groq  # Asegurate de tener la librería correcta

st.set_page_config(page_title="Chat de IA", page_icon="🥥") # Algo similar a un header
st.title("Aplicación con Streamlit - Maia") # Título de la aplicación

nombre = st.text_input("Cuál es tu nombre?") # Caja para ingresar texto
if st.button("Saludar!"): # Botón
    st.write(f"Hola {nombre}! Bienvenido") # write Muestra texto, nros, datos y otros elementos en la app


MODELOS = ['llama-3.1-8b-instant', 'llama-3.3-70b-versatile', 'deepseek-r1-distill-llama-70b']

def configurar_pagina():
    st.title("Mi Chat de IA - Maia")
...
def crear_usuario_groq():
    clave_secreta = st.secrets["CLAVE_API"] # Tu clave real
